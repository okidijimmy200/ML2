    ...
    def fit(
        ...
        for i in tqdm(range(self.epochs)):
            ...
            if save_weights and (i + 1) % save_frequency == 0:
                ...
                torch.save(self.generator.state_dict(), f"{save_path}/generator-epoch-{i+1}.h5")
                torch.save(discriminator.state_dict(), f"{save_path}/discriminator-epoch-{i+1}.h5")
                torch.save(classifier.state_dict(), f"{save_path}/classifier-epoch-{i+1}.h5")
...
class CTABGAN:
  ...
  """
    ...
    def generate_samples(self, synthesizer=None, data_prep=None):
        synthetic_data = None
        if synthesizer is not None:
            synthetic_samples = synthesizer.sample(len(self.raw_data))
            synthetic_data = data_prep.inverse_prep(synthetic_samples)
        else:
            synthetic_samples = self.synthesizer.sample(len(self.raw_data))
            synthetic_data = self.data_prep.inverse_prep(synthetic_samples)
        return synthetic_data

    def sample_from_pretrained(self, output_path="./synthetic-dataset.csv"):
        ...
        data = self.generate_samples(synthesizer=data_synthesizer, data_prep=data_prep)
